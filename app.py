...
if not df.empty:
    ...
    view_type = st.sidebar.radio(
        "Select View", ["Player Comparison", "Individual Player"], index=0
    )
    ...
    if view_type == "Individual Player":
        ...
        selected_metric = st.sidebar.selectbox(
            "Select Metric to Compare", all_metrics, index=0
        )
    ...
    if view_type == "Individual Player" and selected_player:
        ...
        with col1:
            ...
            st.metric("Matches Played", matches_played, delta=f"{total_minutes} mins")

        with col2:
            goals = player_data["Gls"].sum()
            xg = player_data["xG"].sum()
            st.metric("Goals", goals, delta=f"xG: {xg:.1f}")

        with col3:
            assists = player_data["Ast"].sum()
            xag = player_data["xAG"].sum()
            st.metric("Assists", assists, delta=f"xAG: {xag:.1f}")

        with col4:
            goal_inv = player_data["Goal_Involvement"].sum()
            shots = player_data["Sh"].sum()
            st.metric("Goal Involvements", goal_inv, delta=f"Shots: {shots}")
        ...
        with col6:
            ...
            st.metric("Shot Creating Actions", sca, delta=f"Goal Creating: {gca}")

        with col7:
            touches = player_data["Touches"].mean()
            carries = player_data["Carries"].mean()
            st.metric("Avg Touches", f"{touches:.1f}", delta=f"Carries: {carries:.1f}")

        with col8:
            prog_carries = player_data["PrgC"].sum()
            prog_passes = player_data["PrgP"].sum()
            st.metric(
                "Progressive Carries", prog_carries, delta=f"Prog Passes: {prog_passes}"
            )
        ...
        with tab1:
            ...
            selected_goal_metrics = st.multiselect(
                "Select Goal Metrics", goals_metrics, default=["Gls", "xG", "Sh"]
            )
        ...
        if selected_goal_metrics:
            with tab1:
                ...
                chart1 = (
                    ...
                    .properties(height=400, title="Goal Involvement Metrics")
                    .interactive()
                )
        ...
        if selected_prog_metrics:
            with tab2:
                ...
                chart2 = (
                    ...
                    .properties(height=400, title="Progression Metrics")
                    .interactive()
                )
        ...
        if selected_def_metrics:
            with tab3:
                ...
                chart3 = (
                    ...
                    .properties(height=400, title="Defensive Metrics")
                    .interactive()
                )
                ...
                comparison_data.append(
                    {
                        ...
                        "Game Value": row[
                            selected_metric
                        ],  # Individual game value for tooltip
                    }
                )
        ...
        st.dataframe(
            summary_stats.sort_values(("Total"), ascending=False), width="stretch"
        )
